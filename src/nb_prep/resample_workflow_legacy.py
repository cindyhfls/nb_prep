...
from .volume import mni_affine, mni_coords, find_truncation_boundaries, canonical_volume_coords, aseg_mapping, extract_data_in_mni,resample_mni_to_resolution
...
def workflow_single_run(label, sid, wf_root, out_dir, combinations, subj,
    ...
    for mm in [2]: # was [2, 3, 4]
        ...
        if out_fns and all([os.path.exists(_) for _ in out_fns]):
        ...
        combine_funcs.append(None)
        if save_mni_nifti:
            out_mni_nifti = f'{out_dir}/{space}/Nifti/{tag}/sub-{sid}_{label}_space-MNI152NLin2009cAsym_res-{mm:02d}.nii.gz'
            todo.append(mm)
    ...
    if todo:
        ...
        for mm, res in zip(todo, output):
            ...
            if isinstance(res, dict):
                ...
                out_fn = out_mni_nifti
                img = nib.Nifti1Image(res[0], res[1]) # res tuple output from resample_mni_to_resolution (data,affine)
    ...
    tag = '1step_linear_overlap'
    space = 'canonical'
    out_fn = f'{out_dir}/{space}/Nifti/{tag}/sub-{sid}_{label}_space-canonical.nii.gz'
    if not os.path.exists(out_fn):
        os.makedirs(os.path.dirname(out_fn), exist_ok=True)
        ...
        img = nib.Nifti1Image(output[0], affine=subj.canonical_affine)
        img.to_filename(out_fn)
        del img,output
        gc.collect()
    # out_fn = f'{out_dir}/{space}/average-volume/{tag}/sub-{sid}_{label}.nii.gz'
    ...
    #         if save_mni_nifti:
    #             output.append(extract_data_in_mni(d, mm=mm, cortex=False,atlas_names=atlas_names,mni_out_fn = out_mni_nifti))
    #         else:
    #             output.append(extract_data_in_mni(d, mm=mm, cortex=False,atlas_names=atlas_names,mni_out_fn = out_mni_nifti))
...
def resample_workflow(
    ...
    for label in labels:
        workflow_single_run(label, sid, wf_root, out_dir, combinations, subj, n_jobs=n_jobs,atlas_names=atlas_names,save_mni_nifti=save_mni_nifti)
